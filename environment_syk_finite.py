...
class CircuitEnv():
    ...
    def __init__(self, conf, device):
        ...
        self.min_eig = self.helmholtz_free_energy()
    ...
    def step(self, action, train_flag = True) :
        ...
        if ctrl < self.num_qubits:
            next_state[6+gate_tensor][targ][ctrl] = 1
        elif rot_qubit < self.num_qubits:
            next_state[6+gate_tensor][self.num_qubits+rot_axis-1][rot_qubit] = 1
        ...
        # circuit_drawer(self.make_circuit())
        # print(self.error)
        # print('---------------')
        save_circ_init = 0 #self.make_circuit()
        self.save_circ = 0 #save_circ_init.to_json()
        
        rwd = self.reward_fn(energy)
        self.prev_energy = np.copy(energy)
        if rwd == 5:
            self.free_en_state = np.abs(free_en_state)
            # print(self.free_en_state)
            # exit()
        else:
    ...
    def get_energy(self, thetas=None):
        ...
        free_energy_noisy, expval, entropy, psi  = vc.get_free_energy(self.num_qubits,noisy_circ,self.hamiltonian, inverse_temp=self.inverse_temp)
        free_energy_noiseless, expval, entropy, psi = vc.get_free_energy(self.num_qubits,circ,self.hamiltonian,inverse_temp=self.inverse_temp)         
        free_energy = free_energy_noisy + self.energy_shift
        free_energy_noiseless = free_energy_noiseless  + self.energy_shift
        return free_energy, free_energy_noiseless, expval, entropy, psi
